...
minscore = min(lowest[end[1]][end[0]])

def backtrack(moving, newpos, scorechange):
    if grid[newpos[1]][newpos[0]] != "#":
        if lowest[newpos[1]][newpos[0]][newpos[2]] == lowest[moving[1]][moving[0]][moving[2]] + scorechange:
            return newpos
    return False


bestseats = set()
for dir, final in enumerate(lowest[end[1]][end[0]]):
    if final == minscore:
        queue.append((end[0], end[1], dir))

while len(queue) > 0:
    moving = queue.pop(0)
    bestseats.add((moving[0], moving[1]))
    rightturn = backtrack(moving, (moving[0], moving[1], (moving[2] + 1) % 4), -1000)
    leftturn = backtrack(moving, (moving[0], moving[1], (moving[2] - 1) % 4), -1000)
    movefwd = backtrack(moving, (moving[0] - dirs[moving[2]][0], moving[1] - dirs[moving[2]][1], moving[2]), -1)
    if rightturn:
        queue.append(rightturn)
    if leftturn:
        queue.append(leftturn)
